...
参考实现见 [`references/example.ts`](references/example.ts) —— 那是**形态参考**,不是要求用 TypeScript。
...
读用户的项目和需求,选**最贴合的语言**,照 `references/example.ts` 的结构翻译。常见映射:
...
1. **machine 定义 = 纯数据。** 只有「状态有哪些」「什么事件到哪个状态」。可内联,也可拆成独立的 `machine.json` / dict / map。不掺任何逻辑。
...
这是长期可读性的核心卖点,**必须真正实现**,不能留空。规范(实现以 `references/example.ts` 的 `render()` 为准):
