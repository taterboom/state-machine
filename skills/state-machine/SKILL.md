...
2. **转移引擎 = 纯校验,决策在调用处。** 调用方直接指定目标状态 `transition(target)`,引擎只做:查白名单 → 合法就换状态 → 交给 log。**状态机不替你决定去哪**(没有事件层、没有 `onEnter` 里的隐藏分支),它只回答「这一步合不合法」。不含任何业务副作用。
...
```
const r = m.transition('creating', payload)
