...
// 约定:调用方直接说去哪个状态 transition(target),状态机只校验合不合法;
...
const r = order.transition('creating', { bar: 'x' })
if (!r.accepted) {
  // 唯一的错误处理通道(非法跳转)
}
